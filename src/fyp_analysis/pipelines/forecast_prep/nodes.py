...
from .core import get_possible_endog_variables as _get_possible_endog_variables
...
def get_grangers_matrix(
    ...
    The columns are the response variable, rows are predictors.

    The values in the table are the P-Values. P-Values lesser than the
    significance level (0.05), implies the Null Hypothesis that the
    coefficients of the corresponding past values is zero, that is, the X does
    not cause Y can be rejected.
    ...
    -------
    grangers
        the grangers correlation matrix, which holds the p-values for the test
        between variables
    """
...
def get_possible_endog_variables(
    ...
    """
    Get possible endog variables for tax base features, based on Granger
    causality.

    Return a dict of possible endog variables based on Grangers causality for
    each tax base feature.
    ...
    Returns
    -------
    A list of variable names that are Granger significant for each tax base
    feature
    """
    out = {}
