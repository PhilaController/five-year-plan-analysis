...
wheres = [f"(document_type = '{deed_type}')" for deed_type in deed_types]
where = " OR ".join(wheres)

# Load the transfers data
rtt = carto2gpd.get(
    "https://phl.carto.com/api/v2/sql",
    table_name="rtt_summary",
    fields=[
        "opa_account_num",
        "local_tax_amount",
        "document_id",
...
# Convert to date
rtt["recording_date"] = pd.to_datetime(
    rtt["recording_date"], errors="coerce"
).dt.tz_localize(None)

# Drop things that aren't taxed and only keep one per transaction
deeds = rtt.dropna(subset=["local_tax_amount"]).drop_duplicates(subset=["document_id"])
```
...
with plt.style.context(get_theme()):
    ...
    plt.savefig("figures/residential-sales-count.png")
    plt.show()
```
...
with plt.style.context(get_theme()):
    ...
    plt.savefig("figures/residential-revenue.png")
    plt.show()
```
...
```python
cbo = load_cbo_data(date="latest", raw=True).assign(
...
model, residential_sales = get_forecast(
    res,
    "num_records",
    cbo_regressor="res_fixed_invest",
...
model, nonresidential_sales = get_forecast(
    ...
    abatement_dates=[("2022-01-01", "2023-01-01")],
